...
export default function HeroCarousel() {
  ...
  const slides = [
    ...
    },
    {
      ...
      statusText: "\"Alle Serien sind online. Neues Video zum Epislon-N Kriterium.\"",
      primaryAction: { text: "KURS ÖFFNEN", href: "/Kurse/Analysis_1" },
      secondaryAction: { text: "QUIZ STARTEN", href: "/Quiz/Analysis_1" },
  ...
  // Helper to render a single slide's content
  const renderSlideContent = (slide: typeof slides[0]) => (
    <>
      {/* Left: Content */}
      <div className="p-8 md:p-12 md:w-3/5 z-10 flex flex-col justify-between h-full pl-16 shrink-0">
        <div>
        ...
        <div className={`flex flex-wrap gap-4 ${slide.primaryAction.disabled ? 'grayscale opacity-60 pointer-events-none' : ''}`}>
          ...
          {slide.secondaryAction.href ? (
            <Link
              href={slide.secondaryAction.href}
              className="px-6 py-3 border border-surface-light text-white font-bold rounded hover:bg-math-magenta/20 hover:border-math-magenta transition-all duration-300 ease-in-out flex items-center gap-2"
            >
      ...
      <div className="md:w-2/5 min-h-[300px] bg-black border-l border-surface-light flex items-center justify-center relative overflow-hidden shrink-0">
  ...
  return (
    <div 
      className="relative group rounded-2xl border border-surface-light bg-surface overflow-hidden"
    ...
    >
      ...
      >
        {slides.map((slide) => (
          <div 
            key={slide.id}
            className="flex flex-col md:flex-row min-h-[450px] w-full shrink-0"
          >
