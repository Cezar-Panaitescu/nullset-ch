...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "KURSE", href: "/Analysis_1" },
    { name: "ARCHIV", href: "#" },
  ];

  return (
    // UPDATED: bg-white/[0.02] creates the glass tint. border-white/10 makes the edge visible.
    <div className="fixed top-0 left-0 w-full px-6 py-4 flex justify-between items-center text-xs font-mono text-gray-400 bg-white/[0.02] backdrop-blur-md border-b border-white/10 z-50 transition-all duration-300 hover:bg-white/[0.04]">
      ...
      <nav className="flex items-center gap-8">
        {navLinks.map((link) => (
          <Link 
            key={link.name} 
            href={link.href}
            className="hover:text-white transition-colors tracking-widest relative group"
          >
            {link.name}
